...
from prompting.zero_shot import get_zero_shot_prediction_single_category
...
def evaluate_zero_shot(data_path: str, api_key: str, output_dir: str = "results/zero_shot", limit: int = None):
    """Evaluate Zero-shot prompting technique on Bloom taxonomy dataset."""
    ...
    for seq, (_, row) in enumerate(df.iterrows(), start=1):
        ...
        try:
            ...
            # Get model predictions with Zero-shot
    ...
    # Print results
    print_detailed_results(metrics, categories, 'Zero-shot')
...
if __name__ == "__main__":
    ...
    # Run evaluation
    try:
        print("\nStarting Zero-shot evaluation on Bloom taxonomy dataset...")

# bloom_taxonomy_evaluation/evaluation/evaluate_cot.py
...
from prompting.cot import get_cot_prediction_binary_class, get_cot_prediction_single_category
...
def evaluate_cot(data_path: str, api_key: str, output_dir: str = "results/cot", limit: int = None):
    """Evaluate Chain of Thought prompting technique on Bloom taxonomy dataset with binary predictions."""
    ...
    for seq, (_, row) in enumerate(df.iterrows(), start=1):
        ...
        try:
            ...
            # Get model predictions with Chain of Thought (binary)
            model_binary = get_cot_prediction_binary_class(
            ...
            model_category = get_cot_prediction_single_category(
                learning_outcome=row['learning_outcome'],
                client=client
            )
    ...
    plot_category_performance(
        metrics, 
        categories, 
        'Chain of Thought (Binary)',
        f"{output_dir}/cot_performance.png"
    )
    
    # Print results
    print_detailed_results(metrics, categories, 'Chain of Thought (Binary)')
    ...
    metrics_summary = {
        'technique': 'Chain of Thought (Binary)',
...
if __name__ == "__main__":
    ...
    # Run evaluation
    try:
        print(f"\nStarting Chain of Thought evaluation on Bloom taxonomy dataset (Binary Classification)...")
        print(f"Using data file: {config.DATA_PATH}")
        
        results, metrics = evaluate_cot(

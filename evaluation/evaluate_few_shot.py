# bloom_taxonomy_evaluation/evaluation/evaluate_few_shot.py
...
from prompting.few_shot import get_few_shot_prediction_binary_class, get_few_shot_prediction_single_category
...
def evaluate_few_shot(data_path: str, api_key: str, output_dir: str = "results/few_shot", limit: int = None):
    """Evaluate Few-shot prompting technique on Bloom taxonomy dataset with binary predictions."""
    ...
    for seq, (_, row) in enumerate(df.iterrows(), start=1):
        ...
        try:
            ...
            # Get model predictions with Few-shot (binary)
            model_binary = get_few_shot_prediction_binary_class(
            ...
            model_category = get_few_shot_prediction_single_category(
                learning_outcome=row['learning_outcome'],
                client=client
            )
    ...
    # Print results
    print_detailed_results(metrics, categories, 'Few-shot (Binary)')
...
if __name__ == "__main__":
    ...
    # Run evaluation
    try:
        print(f"\nStarting Few-shot evaluation on Bloom taxonomy dataset (Binary Classification)...")

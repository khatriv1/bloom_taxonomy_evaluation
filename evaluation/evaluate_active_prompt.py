...
from prompting.active_prompt import get_active_prompt_prediction_binary_class, get_active_prompt_prediction_single_category
...
def evaluate_active_prompt(data_path: str, api_key: str, output_dir: str = "results/active_prompt", limit: int = None):
    """Evaluate Active Prompting prompting technique on Bloom taxonomy dataset with binary predictions."""
    ...
    for seq, (_, row) in enumerate(df.iterrows(), start=1):
        ...
        try:
            ...
            # Get model predictions with Active Prompting (binary)
    ...
    plot_category_performance(
        metrics, 
        categories, 
        'Active Prompting (Binary)',
        f"{output_dir}/active_prompt_performance.png"
    )
    
    # Print results
    print_detailed_results(metrics, categories, 'Active Prompting (Binary)')
    ...
    metrics_summary = {
        'technique': 'Active Prompting (Binary)',
...
if __name__ == "__main__":
    ...
    # Run evaluation
    try:
        print(f"\nStarting Active Prompting evaluation on Bloom taxonomy dataset (Binary Classification)...")

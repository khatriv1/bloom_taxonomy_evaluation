# bloom_taxonomy_evaluation/evaluation/evaluate_contrastive_cot.py
...
from prompting.contrastive_cot import get_contrastive_cot_prediction_binary_class, get_contrastive_cot_prediction_single_category
...
def evaluate_contrastive_cot(data_path: str, api_key: str, output_dir: str = "results/contrastive_cot", limit: int = None):
    """Evaluate Contrastive CoT prompting technique on Bloom taxonomy dataset with binary predictions."""
    ...
    for seq, (_, row) in enumerate(df.iterrows(), start=1):
        ...
        try:
            ...
            # Get model predictions with Contrastive CoT (binary)
            model_binary = get_contrastive_cot_prediction_binary_class(
            ...
            model_category = get_contrastive_cot_prediction_single_category(
                learning_outcome=row['learning_outcome'],
                client=client
            )
    ...
    plot_category_performance(
        metrics, 
        categories, 
        'Contrastive CoT (Binary)',
        f"{output_dir}/contrastive_cot_performance.png"
    )
    
    # Print results
    print_detailed_results(metrics, categories, 'Contrastive CoT (Binary)')
    ...
    metrics_summary = {
        'technique': 'Contrastive CoT (Binary)',
...
if __name__ == "__main__":
    ...
    # Run evaluation
    try:
        print(f"\nStarting Contrastive CoT evaluation on Bloom taxonomy dataset (Binary Classification)...")
        print(f"Using data file: {config.DATA_PATH}")
        
        results, metrics = evaluate_contrastive_cot(

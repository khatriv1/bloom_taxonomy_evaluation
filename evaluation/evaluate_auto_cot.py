# bloom_taxonomy_evaluation/evaluation/evaluate_auto_cot.py
...
from prompting.auto_cot import get_auto_cot_prediction_binary_class, get_auto_cot_prediction_single_category
...
def evaluate_auto_cot(data_path: str, api_key: str, output_dir: str = "results/auto_cot", limit: int = None):
    """Evaluate Auto-CoT prompting technique on Bloom taxonomy dataset with binary predictions."""
    ...
    for seq, (_, row) in enumerate(df.iterrows(), start=1):
        ...
        try:
            ...
            # Get model predictions with Auto-CoT (binary)
            model_binary = get_auto_cot_prediction_binary_class(
            ...
            model_category = get_auto_cot_prediction_single_category(
                learning_outcome=row['learning_outcome'],
                client=client
            )
    ...
    plot_category_performance(
        ...
        categories, 
        'Auto-CoT (Binary)',
        f"{output_dir}/auto_cot_performance.png"
    )
    
    # Print results
    print_detailed_results(metrics, categories, 'Auto-CoT (Binary)')
...
if __name__ == "__main__":
    ...
    # Run evaluation
    try:
        print(f"\nStarting Auto-CoT evaluation on Bloom taxonomy dataset (Binary Classification)...")

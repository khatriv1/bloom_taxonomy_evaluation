...
from prompting.self_consistency import get_self_consistency_prediction_binary_class, get_self_consistency_prediction_single_category
...
def evaluate_self_consistency(data_path: str, api_key: str, output_dir: str = "results/self_consistency", limit: int = None, n_samples: int = 3):
    """Evaluate Self-Consistency prompting technique on Bloom taxonomy dataset with binary predictions."""
    ...
    for seq, (_, row) in enumerate(df.iterrows(), start=1):
        ...
        try:
            ...
            # Get model predictions with Self-Consistency (binary)
            model_binary = get_self_consistency_prediction_binary_class(
    ...
    plot_category_performance(
        metrics, 
        categories, 
        'Self-Consistency (Binary)',
        f"{output_dir}/self_consistency_performance.png"
    )
    
    # Print results
    print_detailed_results(metrics, categories, 'Self-Consistency (Binary)')
    ...
    metrics_summary = {
        'technique': 'Self-Consistency (Binary)',
...
if __name__ == "__main__":
    ...
    # Run evaluation
    try:
        print(f"\nStarting Self-Consistency evaluation on Bloom taxonomy dataset (Binary Classification)...")
